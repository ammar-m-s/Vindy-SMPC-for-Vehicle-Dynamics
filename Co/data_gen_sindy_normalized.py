...
Outputs:
    ...
    sindy_data.mat  – MATLAB export
    Diagnostic plots: trajectories, velocities, slip angles, tire forces

Notes on normalization
----------------------
This script now saves BOTH raw physical-unit data and normalized data.
The normalized arrays are dimensionless and use explicit scale factors
stored in the output file so the identification stage can work in a
consistent normalized unit system end-to-end.
"""
...
def build_normalization_scales(params, ctrl, v_refs):
    """Define explicit scales used to make the dataset dimensionless."""
    L_wb = params['lf'] + params['lr']
    v_scale = float(max(v_refs))
    omega_scale = v_scale / L_wb
    accel_scale = v_scale**2 / L_wb
    angle_scale = 1.0          # radians are already natural SI angle units
    torque_scale = ctrl['Tr_max']
    return {
        'vx': v_scale,
        'vy': v_scale,
        'omega': omega_scale,
        ...
        'vx_dot': accel_scale,
        'vy_dot': accel_scale,
        'omega_dot': accel_scale / L_wb,
        'delta': angle_scale,
        'Tr': torque_scale,
    }


def normalize_dataset(X_raw, Xdot_raw, U_raw, scales):
    x_scale = np.array([
        scales['vx'], scales['vy'], scales['omega'],
        scales['alpha_f'], scales['alpha_r'],
    ])
    xdot_scale = np.array([
        scales['vx_dot'], scales['vy_dot'], scales['omega_dot'],
    ])
    u_scale = np.array([
        scales['delta'], scales['Tr'],
    ])
    return X_raw / x_scale, Xdot_raw / xdot_scale, U_raw / u_scale, x_scale, xdot_scale, u_scale
...
v_refs = [10, 15, 20, 25]
scales = build_normalization_scales(params, ctrl, v_refs)
...
for k, v in scales.items():
    unit = {
        'vx': 'm/s', 'vy': 'm/s', 'omega': 'rad/s', 'alpha_f': 'rad', 'alpha_r': 'rad',
        'vx_dot': 'm/s^2', 'vy_dot': 'm/s^2', 'omega_dot': 'rad/s^2', 'delta': 'rad', 'Tr': 'Nm'
    }[k]
    print(f"  {k:<8} = {v:9.4f} [{unit}]")
print()
...
for d in all_data:
    ...
    X_sindy_raw.append(np.column_stack([
        d['X'][3, idx],
        d['X'][4, idx],
        d['X'][5, idx],
        d['alpha'][0, idx],
    ...
    Xdot_sindy_raw.append(np.column_stack([
        d['Xdot'][3, idx],
        d['Xdot'][4, idx],
        d['Xdot'][5, idx],
    ]))
    U_sindy_raw.append(np.column_stack([
        d['X'][6, idx],
        d['X'][7, idx],
    ]))

X_sindy_raw    = np.vstack(X_sindy_raw)
Xdot_sindy_raw = np.vstack(Xdot_sindy_raw)
U_sindy_raw    = np.vstack(U_sindy_raw)

X_sindy, Xdot_sindy, U_sindy, x_scale, xdot_scale, u_scale = normalize_dataset(
    X_sindy_raw, Xdot_sindy_raw, U_sindy_raw, scales)

print("=== SINDy Dataset ===")
print(f"  {X_sindy.shape[0]} samples")
print("  Raw X:    [vx, vy, omega, alpha_f, alpha_r]")
print("  Raw Xdot: [vx_dot, vy_dot, omega_dot]")
print("  Raw U:    [delta, Tr]")
print("  Saved normalized arrays with matching scale vectors.")

np.savez(
    'sindy_data_normalized.npz',
    X_sindy=X_sindy,
    Xdot_sindy=Xdot_sindy,
    U_sindy=U_sindy,
    X_sindy_raw=X_sindy_raw,
    Xdot_sindy_raw=Xdot_sindy_raw,
    U_sindy_raw=U_sindy_raw,
    x_scale=x_scale,
    xdot_scale=xdot_scale,
    u_scale=u_scale,
    scale_names_X=np.array(['vx', 'vy', 'omega', 'alpha_f', 'alpha_r'], dtype='U16'),
    scale_names_Xdot=np.array(['vx_dot', 'vy_dot', 'omega_dot'], dtype='U16'),
    scale_names_U=np.array(['delta', 'Tr'], dtype='U16'),
)

savemat('sindy_data_normalized.mat', {
    'X_sindy': X_sindy,
    'Xdot_sindy': Xdot_sindy,
    'U_sindy': U_sindy,
    'X_sindy_raw': X_sindy_raw,
...
print("  Saved to sindy_data_normalized.npz and sindy_data_normalized.mat\n")
...
rows    = [3, 4, 5]
scales_plot  = [1.0, 1.0, 180.0/np.pi]
for j, (ax, label, row, sc) in enumerate(zip(axes, ylabels, rows, scales_plot)):
    for iv, d in enumerate(all_data):
        ax.plot(d['t'], d['X'][row] * sc, color=colors[iv], lw=1, label=f"{d['v_ref']} m/s")
...
for j, (ax, label) in enumerate(zip(axes, ['$\alpha_f$ [deg]', '$\alpha_r$ [deg]'])):
    for iv, d in enumerate(all_data):
        ax.plot(d['t'], d['alpha'][j] * 180/np.pi, color=colors[iv], lw=1, label=f"{d['v_ref']} m/s")
    ax.set_ylabel(label); ax.grid(True)
    if j == 0:
        ax.legend(loc='best', fontsize=8)
    else:
...
for ax, key, flabel, title in zip(
    ...
    for iv, d in enumerate(all_data):
        ...
        ax.scatter(d['alpha'][alpha_idx, pidx]*180/np.pi, d[key][pidx], s=5, color=colors[iv], label=f"{d['v_ref']} m/s")
...
for iv, d in enumerate(all_data):
    axes[0].plot(d['t'], d['X'][6]*180/np.pi, color=colors[iv], lw=1, label=f"{d['v_ref']} m/s")
...
for d in all_data:
    ...
    print(f"  {d['v_ref']:2d} m/s ({d['laps']} laps):  alpha_f=[{af.min():+.1f}, {af.max():+.1f}] deg   alpha_r=[{ar.min():+.1f}, {ar.max():+.1f}] deg")
